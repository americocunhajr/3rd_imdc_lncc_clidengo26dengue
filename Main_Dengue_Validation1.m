...
% -----------------------------------------------------------
case_name = 'IMDC2026_Dengue_Validation1';
...
FileNameCSV_inp = 'IMDC2026_AggregatedData-Dengue_';
 DirNameCSV_out = 'Dengue_Validation1_2022–2023-Season';
FileNameCSV_out = 'IMDC2026_Dengue_Validation1_';
% -----------------------------------------------------------
...
for j = 1:Nufs
    ...
    % Denoise cases time series
    polyOrder = 3;
    frameLen  = 7;   % must be odd
    %DataRaw.cases = DenoiseSG(DataRaw.cases,polyOrder,frameLen);
    %DataRaw.cases = RemoveNonNegativeValues(DataRaw.cases);
    ...
    % % Denoise epidemic time series
    % polyOrder = 3;
    % frameLen  = 7;   % must be odd
    % for s = 1:nYearsTrain
    %     DataTrain_incidence(:,s) = ...
    %         DenoiseSG(DataTrain_incidence(:,s),polyOrder,frameLen);
    %     DataTrain_incidence(:,s) = ...
    %         RemoveNonNegativeValues(DataTrain_incidence(:,s));
    % end

    DataTrain_prevalence = cumsum(DataTrain_incidence);

    % synthetic data
    %DataTrain_incidence = CasesGen(DataTrain_incidence,nReal);
...
%          lag_min   lag_min  lag_min lag_max   lag_max  lag_max]';
...
%          lag_min   lag_min  lag_min lag_max   lag_max  lag_max]';
    ...
    graphObj1.title     = 'Validation Test 1: 2022-2023 Season';
    ...
    graphObj2.title     = 'Validation Test 1: 2022-2023 Season';
